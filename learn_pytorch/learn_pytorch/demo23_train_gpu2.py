...
# 定义训练设备
device = torch.device("cuda")  # 对于多GPU
# device = torch.device("cuda:0")  # 指定使用GPU 0
# device = torch.device("cuda:1")  # 指定使用GPU 1
# device = torch.device("cuda:0,1")  # 指定使用GPU 0 和 GPU 1
# 另一种写法 device = torch.cuda.device("cuda" if torch.cuda.is_available() else "cpu")
...
loss_fn = loss_fn.to(device) # 将损失函数加载到GPU中
# 创建优化器
lr = 1e-2
# 1e-2 = 1 × 10^-2 = 0.01
...
for i in range(epoch):
    ...
    for data in dataloader_train:
        imgs, targets = data
        imgs = imgs.to(device)  # 将数据加载到GPU中
        targets = targets.to(device)  # 将标签加载到GPU中
        outputs = (model(imgs))
